...
def find_class_id(names, target_class: str) -> int | None:
    """
    names puede ser dict {0:'a',1:'b'} o list ['a','b']
    """
    t = norm(target_class)
...
def main():
    ...
    if not runs_index.exists():
        raise FileNotFoundError(
            f"No encuentro {runs_index}.\n"
            f"Ejecuta antes make_yolo_dataset_exp2.py --stage {stage}"
        )

    out_dir = meta_dir / "metrics"
    out_dir.mkdir(parents=True, exist_ok=True)

    out_all = out_dir / f"metrics_{args.weights}_all_{args.split}.csv"
    out_focus = out_dir / f"metrics_{args.weights}_focus_{args.split}.csv"
    ...
    for _, r in df_runs.iterrows():
        ...
        if not wfile.exists():
            print(f"[AVISO] No existe {args.weights}.pt: {wfile}")
            continue
        if not dataset_yaml.exists():
            print(f"[AVISO] No existe dataset_yaml: {dataset_yaml}")
            continue

        print(
            f"\n[VAL Exp2] stage={stage} | {variant} seed={seed} freeze={freeze} "
            f"weights={args.weights}.pt split={args.split}"
        )
        ...
        if cid is None:
            print(f"[AVISO] La clase '{args.target_class}' no está en names: {metrics.names}")
            continue
