...
def main():
    ...
    ap.add_argument(
        ...
        type=float,
        default=None,
        help="Confidence threshold for val (e.g., 0.25). If omitted uses default.",
    )
    ...
    if not runs_index.exists():
        raise FileNotFoundError(
            f"No encuentro {runs_index}. Ejecuta make_yolo_dataset_exp4.py antes."
        )
    ...
    for _, r in df_runs.iterrows():
        ...
        try:
            imgsz = int(r.get("imgsz"))
        except Exception:
            imgsz = int(cfg.get("yolo", {}).get("imgsz", 960))
        ...
        print(
            f"\n[VAL exp4] stage={stage} | {variant} seed={seed} freeze={freeze} "
            f"weights={args.weights}.pt split={args.split} conf={args.conf} imgsz={imgsz}"
        )
