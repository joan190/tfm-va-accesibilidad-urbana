...
# Evita backend GUI (Tkinter) al ejecutar por terminal
import matplotlib
matplotlib.use("Agg")
...
def es_title_exp3(stage: str, source: str, split: str, tag: str) -> str:
    return f"Exp3 ({stage}/{source}/{split}/{tag})"


def es_title_bars(stage: str, source: str, split: str, tag: str) -> str:
    return f"{es_title_exp3(stage, source, split, tag)} — Data Augmentation policies (media±desv. típ.)"
...
def es_title_tradeoff(stage: str, source: str, split: str, tag: str, x_m: str, y_m: str) -> str:
    return (
        f"{es_title_exp3(stage, source, split, tag)} — Compromiso "
...
def order_variants_by_ranking(
    df_summary: pd.DataFrame, metrics_dir: Path, source: str, split: str, tag: str
) -> list[str]:
...
def plot_learning_curves_pro(
    ...
    if not rows:
        print("[AVISO] No he podido cargar results.csv para curvas.")
        return
    ...
    if not available:
        print("[AVISO] No encuentro columnas típicas para curvas.")
    ...
    for col in available:
        agg = (
            ...
            .agg(["mean", "std"])
            .reset_index()
            .sort_values(["variant", "epoch"])
        )

        groups = list(agg.groupby(["variant"]).groups.keys())
        colors = get_color_palette(len(groups))

        fig, ax = plt.subplots(figsize=(6.4, 2.8))

        for i, (variant, sub) in enumerate(agg.groupby(["variant"])):
        ...
        out = plots_dir / f"exp3_{stage}_curve_{sanitize(col)}.png"
        save_fig(fig, out)
        print(f"[OK] {_with_color_suffix(out.with_suffix('')).with_suffix('.png')}")


def plot_tradeoff_scatter_means_exp3(
...
def main():
    ...
    if not summary_csv.exists():
        raise FileNotFoundError(
            f"No encuentro {summary_csv}. Ejecuta aggregate_exp3_results.py primero."
        )
    ...
    for m, pretty in barplots:
        ...
        stds = (
            df_sum[m_std].fillna(0.0).astype(float).values
            if m_std in df_sum.columns
            else [0.0] * len(means)
        )

        out_path = plots_dir / f"exp3_{stage}_{args.source}_{args.split}_{tag}_{sanitize(m)}_bar.png"
        color_barplot_pro(
            means=means,
            stds=stds,
            labels=labels,
    ...
    if not args.no_paretos:
        out_path = plots_dir / f"exp3_{stage}_{args.source}_{args.split}_{tag}_tradeoff_custom.png"
        plot_tradeoff_scatter_means_exp3(
            df_summary=df_sum,
            out_path=out_path,
        ...
        for x_m, y_m in tradeoff_pairs:
            out_path = plots_dir / (
                f"exp3_{stage}_{args.source}_{args.split}_{tag}_tradeoff_{sanitize(x_m)}_vs_{sanitize(y_m)}.png"
            )
            plot_tradeoff_scatter_means_exp3(
                df_summary=df_sum,
                out_path=out_path,
    ...
    if args.make_table:
        out_tex = metrics_dir / f"exp3_{stage}_{args.source}_{args.split}_{tag}_table.tex"
