...
def main():
    ...
    src_file = metrics_dir / f"metrics_{args.weights}_{args.source}_{args.split}_{tag}.csv"
    if not src_file.exists():
        raise FileNotFoundError(
            f"No encuentro {src_file}.\n"
            f"Ejecuta antes build_metrics_best_focus_all.py --stage {stage} --split {args.split} --conf {args.conf}"
        )
    ...
    if "variant" not in df.columns:
        raise RuntimeError("Falta columna 'variant' en el CSV. Revisa build_metrics.")
    group_cols = ["variant"]
    ...
    out_path = metrics_dir / f"metrics_summary_{args.source}_{args.split}_{tag}.csv"
